...
function FeesPage() {
  ...
  const allYears = useMemo(
    () => Array.from(new Set(state.feePayments.map((f) => f.academicYear))).sort().reverse(),
    [state.feePayments],
  );
        ...
        feePayments: s.feePayments.map((p) =>
          p.id === paymentId
            ? p.status === "paid"
              ...
              : { ...p, status: "paid", paidOn: new Date().toISOString().split("T")[0], receiptNo: p.receiptNo ?? `RC-${Date.now().toString().slice(-6)}` }
            : p,
        ),
      }),
  ...
  const filteredPayments = state.feePayments.filter((p) => {
    ...
    if (q && !st.name.toLowerCase().includes(q.toLowerCase()) && !st.rollNo.toLowerCase().includes(q.toLowerCase())) return false;
    return true;
  });

  // Per-student quarterly grid
  const studentsForGrid = state.students.filter((s) => classFilter === "all" || s.classId === classFilter)
    .filter((s) => !q || s.name.toLowerCase().includes(q.toLowerCase()) || s.rollNo.toLowerCase().includes(q.toLowerCase()));

  return (
    <div>
      <PageHeader title="Fees" subtitle="Structure, per-student quarterly status and full payment ledger" />
      <div className="space-y-4 p-6">
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          ...
          <Stat label="Paid records" value={state.feePayments.filter((f) => f.status === "paid").length} />
          <Stat label="Pending records" value={state.feePayments.filter((f) => f.status === "pending").length} tone="danger" />
        </div>

        <div className="flex gap-1 rounded-md border border-border bg-muted p-1 w-fit">
          {[["students", "By Student"], ["payments", "All Payments"], ["structure", "Fee Structure"]].map(([k, l]) => (
            <button
              key={k}
        ...
        {tab !== "structure" && (
          <div className="flex flex-wrap items-center gap-2">
            ...
            <select value={year} onChange={(e) => setYear(e.target.value)} className="rounded-md border border-input bg-background px-2 py-1.5 text-sm">
              {allYears.map((y) => <option key={y} value={y}>{y}</option>)}
            </select>
            <select value={quarter} onChange={(e) => setQuarter(e.target.value as typeof quarter)} className="rounded-md border border-input bg-background px-2 py-1.5 text-sm">
              <option value="all">All quarters</option>
              {QUARTERS.map((q) => <option key={q} value={q}>{q}</option>)}
              <option value="admission">Admission</option>
            </select>
            <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as typeof statusFilter)} className="rounded-md border border-input bg-background px-2 py-1.5 text-sm">
              <option value="all">All status</option>
              <option value="paid">Paid</option>
              <option value="pending">Pending / Not paid</option>
            </select>
            <select value={classFilter} onChange={(e) => setClassFilter(e.target.value)} className="rounded-md border border-input bg-background px-2 py-1.5 text-sm">
              ...
              <tbody>
                {state.feeStructures.map((f) => (
                  <tr key={f.id}>
                    ...
                    <td><input type="number" value={f.admissionFee} onChange={(e) => updateFee(f.grade, "admissionFee", Number(e.target.value))} className="w-28 rounded border border-input bg-background px-2 py-1 text-sm tabular-nums" /></td>
                    <td><input type="number" value={f.quarterlyFee} onChange={(e) => updateFee(f.grade, "quarterlyFee", Number(e.target.value))} className="w-28 rounded border border-input bg-background px-2 py-1 text-sm tabular-nums" /></td>
        ...
        {tab === "students" && (
          <Section title="Per-Student Quarterly Status">
            <div className="overflow-x-auto">
              <table className="data-table w-full">
                <thead>
                  <tr><th>Roll</th><th>Student</th><th>Class</th>
                ...
                <tbody>
                  {studentsForGrid.map((s) => {
                    ...
                    const studentPays = state.feePayments.filter((p) => p.studentId === s.id && (!year || p.academicYear === year));
                    const pending = studentPays.filter((p) => p.status === "pending").reduce((sum, p) => sum + p.amount, 0);
                    return (
                      <tr key={s.id}>
                        ...
                        {QUARTERS.map((qk) => {
                          ...
                          if (!p) return <td key={qk} className="text-muted-foreground">—</td>;
                          if (statusFilter !== "all" && p.status !== statusFilter) return <td key={qk} className="text-muted-foreground/40">·</td>;
                          return (
                            <td key={qk}>
                              <button onClick={() => togglePaid(p.id)} title={`Click to mark ${p.status === "paid" ? "pending" : "paid"}`}>
                                {p.status === "paid" ? <Badge tone="success">paid</Badge> : <Badge tone="warning">pending</Badge>}
                              </button>
                            </td>
                          );
                        })}
                        <td className="tabular-nums text-xs">{pending > 0 ? <span className="text-destructive">₹{pending.toLocaleString()}</span> : <span className="text-success">₹0</span>}</td>
                      </tr>
                    );
                  })}
                </tbody>
        ...
        {tab === "payments" && (
          <Section title={`Payments (${filteredPayments.length})`}>
            <div className="overflow-x-auto">
              <table className="data-table w-full">
                <thead>
                  <tr><th>Receipt</th><th>Student</th><th>Class</th><th>Type</th><th>Amount</th><th>Status</th><th>Year</th><th>Date</th></tr>
                </thead>
                <tbody>
